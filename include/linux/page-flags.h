 ...
 *
 * Unfortunately, PageDmaPinned also corresponds to PageTail (the lowest bit in
 * the first union of struct page), so extra caution is required in order to
 * ensure that we only look at non-tail pages.
 */
#define PAGE_DMA_PINNED		0x2
#define PAGE_DMA_PINNED_FLAGS	(PAGE_DMA_PINNED)

/*
 * Because these flags are read outside of a lock, we need to ensure
 * visibility between different threads, so READ|WRITE_ONCE is used.
 */
