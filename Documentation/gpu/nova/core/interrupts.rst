...
A drain fails when an element's framing is bad, which poisons the queue (see
"Draining the GSP-to-CPU queue"). Every later event would fail the
...
Controller, and its API is ABI-stable. Every element opens with the same MCTP
and NVDM transport headers, and the NVDM type in those headers says which kind
of message header follows. An RPC message is a command reply or an unsolicited
event, and its function code says which.
...
that carries an NVDM type the queue does not use, has no trustworthy length, so
the read pointer cannot advance past it. Such a failure poisons the queue:
nova-core logs it once, and every later receive fails with ``EIO`` until the
device is reset.
