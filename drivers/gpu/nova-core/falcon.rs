...
pub(crate) enum FalconDmaSrcOffset {
    ...
    Offset(u32),
    /// DMEM virtual address of the image. The DMA engine tags each loaded block with this value
    /// and adds it to the base address, so the caller sets the base address to account for it.
    DmemVa(u32),
}
...
impl<'a, E: FalconEngine + 'static> Falcon<'a, E> {
    ...
    /// Returns true if the RISC-V core has been suspended.
