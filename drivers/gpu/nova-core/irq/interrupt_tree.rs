...
pub(crate) struct Tree<'a> {
    ...
    rearm_method: PciIrqRearmMethod,
...
impl<'a> Tree<'a> {
    ...
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            serviced,
            msi_type,
            rearm_method: hal.pci_irq_rearm_method(msi_type),
            vectors,
        })
    }
    ...
    pub(super) fn rearm_pci_irq(&self, subtree: Subtree) {
        self.rearm_method.rearm(self.bar, self.serviced, subtree);
    }
    ...
    pub(super) fn trigger(&self, vector: GinVector) -> Result {
        vector.validate(self.num_leaves)?;
        self.bar.write_reg(
    ...
    pub(super) fn disable_all_leaves(&self) {
        for leaf in self.num_leaves.iter() {
    ...
    pub(super) fn reset(&self) {
        ...
        // A `TOP` rearm method leaves the subtrees that it cycles enabled, and a subtree may be
