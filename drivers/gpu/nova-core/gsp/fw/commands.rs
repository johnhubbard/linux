...
/// Power level the GSP is asked to enter.
...
bitfield! {
    /// A GPU's PCI location, encoded the way GSP-RM decodes it. Despite the name GSP-RM gives the
    /// key, the function number is not part of it.
...
impl GspInitRequest {
    ...
    /// Creates the request for `dev`, with the registry keys `regkeys`.
    pub(crate) fn new(
...
impl GspInitResponse {
    ...
    /// Iterates over the FB regions the driver may allocate from: the general-purpose regions
    /// that are not protected and that support both compression and isochronous access.
    ...
    /// including the ones [`Self::usable_fb_regions`] leaves out.
    ///
