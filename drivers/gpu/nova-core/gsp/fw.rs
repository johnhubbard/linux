...
/// Msgq protocol version that the driver speaks.
const MSGQ_VERSION_MAJOR: u16 = 2;
const MSGQ_VERSION_MINOR: u16 = 0;

/// The header that describes a queue to the GSP: the msgq version the driver speaks, and the
/// queue's geometry. The queue pointers are in BAR0 registers rather than in the header.
#[repr(transparent)]
pub(crate) struct MsgqTxHeader(bindings::msgqTxHeader);

impl MsgqTxHeader {
    /// Creates the header of a queue of `msg_count` elements of `msg_size` bytes each, whose
    /// first element starts `entry_off` bytes into the `msgq_size`-byte queue structure.
...
/// The headers of a queue element that carries an RPC message.
#[repr(C)]
...
/// The transport headers that open every queue element, whichever kind of message follows.
///
/// The NVDM type in `nvdm` says which message header follows.
#[repr(C)]
...
impl QueueElementHeader {
    ...
    /// Returns the length of the payload, given the `header_len` bytes of message header before it.
...
/// Header of a GMC (GPU Management Controller) API message.
#[repr(C)]
#[derive(Zeroable)]
pub(crate) struct GmcApiHeader {
    ...
    pub(crate) size: u32,
    /// Copied from a request into its response.
    pub(crate) sequence: u64,
    /// In a request, the largest response the sender accepts. In a response, the `NV_STATUS`.
...
/// GMC event asking the driver to run a high-security binary that GSP-RM has placed in the
/// framebuffer.
pub(crate) const GMCAPI_CMD_EXEC_HS_BINARY: u32 =
    bindings::GMCAPI_COMMANDS_GMCAPI_CMD_EXEC_HS_BINARY;

/// GMC request that tells GSP-RM to suspend. GSP-RM sends no response, and reports the completed
...
/// The headers of a queue element that carries a GMC API message.
#[repr(C)]
...
/// The ucodes image, as GSP-RM's startup arguments describe it.
pub(crate) struct BindataArgs {
    /// DMA address of the level 0 page of the radix3 page table that maps the image.
...
/// Flag asking GSP-RM to place its stack in DMEM.
...
impl GspArgumentsCached {
    /// Creates the arguments for starting the GSP up using `cmdq` as its command queue and
...
impl GspArgumentsPadded {
    ...
    /// Records where the ucodes image is mapped. Must be called before the GSP starts.
    pub(crate) fn set_bindata(this: &Coherent<'_, Self>, bindata: &BindataArgs) {
        io_write!(this, .inner.inner.bindataArgs.radix3, bindata.radix3);
        io_write!(this, .inner.inner.bindataArgs.size, bindata.size);
    }
}
